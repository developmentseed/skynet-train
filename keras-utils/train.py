...
class Upload(keras.callbacks.Callback):
  def on_epoch_end(self, epochs, logs={}):
    if epochs%10 == 0:
      s3.Object('ds-skynet', 'models/dg-goma-2-tuning/' + hyper_name + '/' + str(epochs) + '.hdf5').put(Body=open('./Models/AtrousFCN_Resnet50_16s/checkpoint_weights.hdf5', 'rb'))
...
def train(batch_size, epochs, lr_base, lr_power, weight_decay, classes,
    ...
    resume_training=False, class_weight=None, dataset='SKYNET_DATA',
  ...
  callbacks.append(Upload())
  # set data generator and train
  train_datagen = SegDataGenerator(zoom_range=zoom_range,
    zoom_maintain_shape=zoom_maintain_shape,
    crop_mode=crop_mode,
  ...
  #testimg = ndimage.imread('test.png')
  #testimg2 = ndimage.imread('test2.png')
  #testimg = np.array([testimg, testimg2])
  #print testimg.shape
  #print(model.fit(testimg, batch_size=2))

def getHyperparametersFromSQS():
  queue = sqs.get_queue_by_name(QueueName='hyperparameters.fifo')
  message_recieved = queue.receive_messages()[0]
  hyper_name = message_recieved.md5_of_body
  body = message_recieved.body
  message_recieved.delete()
  s3.Object('ds-skynet', 'models/dg-goma-2-tuning/' + hyper_name + '/hyperparamters.json').put(Body=body)
  return body
...
if __name__ == '__main__':
  ...
  hyperparameters = json.loads(getHyperparametersFromSQS())
  ...
  batchnorm_momentum = hyperparameters['batchnorm_momentum']
  epochs = hyperparameters['epochs']
  lr_base = hyperparameters['lr_base']
  lr_power = hyperparameters['lr_power']
  weight_decay = hyperparameters['weight_decay']
  class_weight[0] = hyperparameters['background_weight']
  vertical_flip = hyperparameters['vertical_flip']
  horizontal_flip = hyperparameters['horizontal_flip']
  zoom_range = hyperparameters['zoom_range']
  zoom_maintain_shape = hyperparameters['zoom_maintain_shape']
  crop_mode = hyperparameters['crop_mode']
  channel_shift_range = hyperparameters['channel_shift_range']
  ...
  train(batch_size, epochs, lr_base, lr_power, weight_decay, classes, model_name, train_file_path, val_file_path,
    ...
    label_suffix=label_suffix, ignore_label=ignore_label, label_cval=label_cval, fill_mode=fill_mode, 
    channel_shift_range=channel_shift_range)
