...
def print_help():
    print """
Usage:
    ./plot_log.sh chart_type[0-%s] /path/to/train.log
Notes:
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        ...
        if not path_to_log.endswith(get_log_file_suffix()):
            print_help()
        suffix = get_chart_type_description(chart_type)
	suffix = suffix.replace(' ', '-').replace('.', '').lower()        
	suffix = '.' + suffix + '.png'
        path_to_png = path_to_log.replace(get_log_file_suffix(), suffix)
        delay = 60
        while True:
            print('%s updating chart' % time.strftime('%Y-%m-%d %H:%M', time.gmtime()))
            plot_chart(chart_type, path_to_png, path_to_log)
            time.sleep(delay)
